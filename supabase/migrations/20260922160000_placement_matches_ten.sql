-- Raises the placement-match requirement from 5 to 10. Only the count
-- required to leave Unranked changes; the underlying mechanics (Elo
-- formula, per-player placement progress, no Demotion Shield during
-- placements) are unchanged. Existing players who already finished
-- placement under the old rule of 5 keep their earned rank -- this only
-- affects players still mid-placement (placement_complete = false) and
-- every new player going forward.
--
-- Builds on the previous migration's fix for the placement-completion
-- constraint race (see 20260922155000): record_match() below writes every
-- field for a player -- elo, win/loss, placement count/completion,
-- resulting rank/RR, Demotion Shield state -- in a single UPDATE, so the
-- row is only ever checked by Postgres in its final, consistent state.
--
...
alter table public.players
  add constraint players_placement_matches_played_check
    check (placement_matches_played between 0 and 10),
  add constraint players_placement_complete_check
    check (placement_complete = (placement_matches_played = 10));
